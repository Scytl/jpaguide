...
Writting unit tests for persistence layer can become a big problem because there are a lot of elements that are controlled or managed by the container.
For example transactions, creation of `EntityManager` or definition of the datasource.
So if you try to create _mocks_ for everything you will end up by re-implementing a _JPA_ provider and/or writting a lot of boilerplate code.
...
.Arquillian
****
*Arquillian* is a testing framework that fills the gap between unit tests and integration tests so writting integration tests is as simple as writing a unit test but running within/against an application server.
...
With _Metamodel API_ you are hidding this details so you can details without affecting the queries.
